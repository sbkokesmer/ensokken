        ...
        <div className="flex-1 space-y-6">
          {cart.length > 0 && (
            <div className="bg-white p-6 rounded-2xl border border-black/5 flex items-center gap-5 shadow-sm">
              <div className="w-12 h-12 rounded-full bg-[#eeebdf] flex items-center justify-center text-black shrink-0">
                <Truck width={24} height={24} />
              </div>
              <div className="flex-1">
                <div className="flex justify-between text-sm font-medium mb-2">
                  <span className="text-black">Gratis verzending</span>
                  {remainingForFreeShipping > 0 ? (
                    <span className="text-zinc-400">nog €{remainingForFreeShipping.toFixed(2)}</span>
                  ) : (
                    <span className="text-emerald-600 font-bold">Bereikt!</span>
                  )}
                </div>
                <div className="h-2 w-full bg-[#eeebdf] rounded-full overflow-hidden">
                  <div
                    className={`h-full transition-all duration-500 rounded-full ${remainingForFreeShipping <= 0 ? 'bg-emerald-500' : 'bg-black'}`}
              ...
              return (
                <div key={item.variantId} className="flex gap-6 p-6 bg-white rounded-3xl border border-black/5 shadow-sm">
                  <div className="w-32 h-32 bg-[#f9f9f9] rounded-2xl flex items-center justify-center shrink-0">
                    <img src={primaryImg} className="w-24 object-contain" alt={item.product.name} />
                  </div>
                  <div className="flex-1 flex flex-col justify-between py-1">
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <h3 className="font-semibold text-lg text-black mb-1">{item.product.name}</h3>
                        <p className="text-sm text-zinc-500">
                          {item.selectedSize}
                          {item.selectedColorName && ` · ${item.selectedColorName}`}
                        </p>
                      </div>
                      <span className="font-bold text-xl text-black">€{(item.product.price * item.quantity).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between items-end">
                      ...
                      <span className="text-sm text-zinc-400">Stukprijs: €{Number(item.product.price).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              );
