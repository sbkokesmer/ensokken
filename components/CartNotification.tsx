  ...
  return (
    <div
      className={`
        ...
        ${notification.show ? "translate-y-0 opacity-100" : "-translate-y-4 opacity-0 pointer-events-none"}
      `}
    >
      <div className="bg-white rounded-2xl shadow-2xl shadow-black/20 border border-black/5 overflow-hidden">
        <div className="p-4">
          <div className="flex items-start justify-between mb-3">
            ...
            <button onClick={closeNotification} className="text-zinc-400 hover:text-black transition-colors p-1 -mr-1">
          ...
          <div className="flex gap-4">
            <div className="w-16 h-20 bg-[#f9f9f9] rounded-lg border border-black/5 flex-shrink-0 p-1">
              <img src={primaryImg} alt={item.product.name} className="w-full h-full object-contain mix-blend-multiply" />
            </div>
            <div className="flex-1 min-w-0 flex flex-col justify-center">
              <h4 className="font-medium text-black text-sm truncate pr-2">{item.product.name}</h4>
              <p className="text-zinc-500 text-xs mt-1 flex items-center gap-1.5">
                <span>{item.selectedSize}</span>
                <span className="w-1 h-1 rounded-full bg-zinc-300" />
                <span className="font-medium text-zinc-700">{item.selectedColorName}</span>
              </p>
              <p className="text-black font-medium text-sm mt-1.5">€{Number(item.product.price).toFixed(2)}</p>
            </div>
          </div>
        ...
        <div className="h-1 w-full bg-zinc-100">
          <div
            className={`h-full bg-black transition-all duration-[4000ms] ease-linear ${notification.show ? 'w-0' : 'w-full'}`}
          />
        </div>
      </div>
    </div>
